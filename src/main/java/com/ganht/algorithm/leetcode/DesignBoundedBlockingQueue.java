 ...
 * void enqueue(int element) Adds an element to the front of the queue. If the queue is full, the calling thread is blocked
 ...
 * that only makes calls to the enqueue method or a consumer thread that only makes calls to the dequeue method. The size
...
public class DesignBoundedBlockingQueue {
    ...
    class BoundedBlockingQueue {
        ...
        private final Queue<Integer> rawQueue = new LinkedList<>();
        ...
        public void enqueue(int element) throws InterruptedException {
            synchronized (lock){
                while(rawQueue.size() >= this.capacity){
                    lock.wait();
                }

                rawQueue.add(element);
                lock.notify();
            }
        }
