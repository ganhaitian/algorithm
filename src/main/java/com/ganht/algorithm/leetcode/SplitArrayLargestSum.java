...
public class SplitArrayLargestSum {
    ...
    public int splitArray(int[] nums, int m) {
        return doSplitArray(nums, 0, m);
    }

    public int doSplitArray(int[] nums, int start, int m) {
        Integer existVal = cache.get(start + "-" + m);
        if (existVal != null) {
            return existVal;
        }
        ...
        int currMin = 0, tmpSum = 0;
        for (int i = start; i < nums.length; i++) {
            tmpSum += nums[i];
            int tmpMin  = doSplitArray(nums, i + 1, m - 1);
            int largest = Math.max(tmpMin, tmpSum);

            if (currMin == 0) {
                currMin = largest;
            } else if (largest > 0 && currMin > largest) {
                currMin = largest;
            }
        }

        cache.put(start + "-" + m, currMin);
        return currMin;
    }
    ...
    public static void main(String[] args){
        ...
        int result = new SplitArrayLargestSum().splitArray(input, 9);
