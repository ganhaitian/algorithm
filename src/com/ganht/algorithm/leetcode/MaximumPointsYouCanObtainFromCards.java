...
public class MaximumPointsYouCanObtainFromCards {
    ...
    public int maxScore(int[] cardPoints, int k) {
        ...
        for (int i = length - k; i < cardPoints.length; i++) {
            sum += cardPoints[i];
        }

        max = sum;
        for (int i = length - k + 1; i <= cardPoints.length; i++) {
            sum = sum - cardPoints[i - 1];
            sum = sum + cardPoints[(i + k - 1) % length];
    ...
    public static void main(String[] args){
        ...
        new MaximumPointsYouCanObtainFromCards().maxScore(input, 3);
    }
