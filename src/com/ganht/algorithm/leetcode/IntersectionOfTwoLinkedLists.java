 ...
 * the head of A, it reads as [0,9,1,2,4]. From the head of B, it reads as [3,2,4]. There are 3 nodes before the intersected
 ...
 */
public class IntersectionOfTwoLinkedLists extends BaseLinkedListProblem {

    // 可以用一个hash表
    // 或者用下面这个，但尽管看着高级，时间复杂度并没有提高多少
    ...
    // which are intersected at node '9'. Since B.length (=4) < A.length (=6), pBpB would reach the end of the merged
    // list first, because pBpB traverses exactly 2 nodes less than pApA does. By redirecting pBpB to head A, and pApA
    // to head B, we now ask pBpB to travel exactly 2 more nodes than pApA would. So in the second iteration, they are
    // guaranteed to reach the intersection node at the same time.
    // If two lists have intersection, then their last nodes must be the same one. So when pApA/pBpB reaches the end of
